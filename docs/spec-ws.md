# spec-ws.md (Дополнение к ТЗ: WebSocket и realtime)

Этот документ описывает WS-протокол и realtime-события как прикладную детализацию `tz_tactical_app.md` для MVP.
Он дополняет ТЗ техническим форматом обмена и не является самостоятельным продуктовым каноном.

Правило приоритета:
- продуктовая семантика каналов/событий определяется `tz_tactical_app.md`;
- данный файл фиксирует транспортный контракт (envelope, eventType, подписки, ошибки);
- при расхождении обновляется `spec-ws.md`, а не ТЗ «под текущую реализацию».

## Масштабирование WebSocket

WebSocket-соединения могут обслуживаться
несколькими backend-инстансами.

Для синхронизации событий используется Redis event bus.

Каждый backend-инстанс:

1. публикует события в Redis
2. подписывается на Redis-каналы
3. ретранслирует события своим WebSocket-клиентам
