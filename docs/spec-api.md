...
- ID: `long`
- Пагинация в MVP может не использоваться по всем endpoint-ам,
  но канонический формат пагинации фиксируется заранее.

  Канонические query params пагинации:
  
  - `page` — номер страницы, начиная с `0`
    - `size` — размер страницы
    - `sort` — поле и направление сортировки, например `createdAt,desc`
  
  Канонический формат paginated response:
  ...
  ```
  Если endpoint в MVP не использует пагинацию,
...
{ "items": [ /* MarkerDto */ ] }
```
В MVP endpoint может использовать непагинируемую выдачу.

При переходе на пагинацию должен использоваться
канонический формат `page/size/sort`
без изменения структуры элемента в `items`
...
В автоматическом режиме клиент должен отправлять координаты
...
### Источник данных (реализация)

Endpoint возвращает read-model **последних известных позиций пользователей**.

Источником ответа может быть Redis-кэш актуального состояния карты.

Backend не обязан каждый раз вычислять последние позиции
через прямое чтение append-only истории `user_geo_locations`
в PostgreSQL.

Типичный алгоритм реализации:

1. определяется список видимых пользователей;
2. последние позиции читаются из Redis;
3. при отсутствии записи выполняется fallback к PostgreSQL;
4. результат может быть записан в Redis для последующих запросов.

Такой подход позволяет снизить нагрузку на PostgreSQL
